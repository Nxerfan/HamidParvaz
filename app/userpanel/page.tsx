...
const jalaali = {
  toJalaali: function (gy: number, gm: number, gd: number): JalaaliResult {
  ...
  toGregorian: function (jy: number, jm: number, jd: number): GregorianResult {
  ...
  isValidJalaaliDate: function (jy: number, jm: number, jd: number): boolean {
    return (
      jy >= -61 &&
      jy <= 3177 &&
      jm >= 1 &&
  ...
  jalCal: function (jy: number): JalCalResult {
    const breaks = [
  ...
  jMonthLength: function (jy: number, jm: number): number {
    if (jm <= 6) return 31;
    if (jm <= 11) return 30;
...
export default function ProfileManagement() {
  ...
  const calculateProgress = () => {
    ...
    let filled = 0;
    fields.forEach((f) => {
      const val = (f as HTMLInputElement | HTMLSelectElement).value.trim();
  ...
  const stringToJDate = useCallback((str: string | null): JalaaliResult | null => {
    if (!str) return null;
  ...
  const openCalendar = useCallback(
    (inputType: string) => {
      let targetDate: JalaaliResult | null = passportExpiryDate
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (
        ...
        (e.target as HTMLElement).id !== "expiryDateInput"
      ) {
        closeCalendar();
      }
    };
  ...
  return (
    <>
            ...
            <div className="ProfileHeader">
              <div className="ProgressWrap">
                ...
                <div className="Avatar">
                  <Image src={PAGE_DATA.avatarSrc} alt="User" width={40} height={40} style={{ borderRadius: "50%" }} />
                </div>
              </div>
      ...
      <style jsx>{`
        ...
        .calendarPopup.show { display: flex; }
        .calendarHeader {
          display: flex;
        ...
        .calendarNavBtn:hover { background-color: var(--grayBg); }
        .calendarTitle {
          font-weight: bold;
          font-size: 16px;
          color: var(--goldText);
        ...
        .calendarDay:not(.empty):hover { background-color: var(--grayBgHover); }
        ...
        .yearsWrapper::-webkit-scrollbar { width: 6px; }
